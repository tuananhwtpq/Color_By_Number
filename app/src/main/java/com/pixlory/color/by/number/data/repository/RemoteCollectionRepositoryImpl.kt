...
import com.pixlory.color.by.number.data.remote.RemoteLevelMapper
import com.pixlory.color.by.number.data.remote.RemoteLevelMetadataLoader
import com.pixlory.color.by.number.data.remote.requireSuccessfulBody
...
class RemoteCollectionRepositoryImpl(
    ...
    private suspend fun collectionsWithLevelCounts(groups: List<com.pixlory.color.by.number.data.remote.RemoteGroupDto>): List<AlbumCollection> =
        coroutineScope {
            groups.map { group ->
                async {
